...
class FamiliesController < ApplicationController
  ...
  def create
    @family = current_user.created_families.build(params[:family])
  ...
  def search
    ...
    @whole_people = @search.relation.select("DISTINCT(families.id), families.*").map(&:member_counter).compact.sum
    @whole_children = @search.relation.select("DISTINCT(families.id), families.*").map(&:children_counter).compact.sum
