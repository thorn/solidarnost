...
class News < ActiveRecord::Base
  ...
  def self.create_about(object)
    if object.class == Family
      attributes = {title: "Создана <a href='/families/#{object.id}'>семья #{object.id} - #{object.title} c приоритетом #{object.priority}", text: "Ссылка: <a href='/families/#{object.id}'>#{object.title}</a>", news_type: 0 }
